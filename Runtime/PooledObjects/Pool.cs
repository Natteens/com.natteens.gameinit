...
namespace GameInit.PooledObjects
{
    ...
    public static class Pool
    {
        ...
        /// <summary>
        /// Executa a inicialização completa do sistema
        /// </summary>
        static void PerformSystemInitialization()
        {
            ForceShutdown();
        ...
        static void OnSceneLoaded(Scene scene, LoadSceneMode mode)
        {
            ...
            foreach (var poolConfig in database.pools)
            {
                if (poolConfig.clearOnSceneLoad && pools.ContainsKey(poolConfig.poolTag))
                {
                    pools[poolConfig.poolTag].DespawnAll();
                    poolsCleared++;
                }
            }
