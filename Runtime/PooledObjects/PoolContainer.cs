...
namespace GameInit.PooledObjects
{
    ...
    public sealed class PoolContainer
    {
        ...
        void ValidateConfiguration(PoolDatabase.PoolConfiguration configuration)
        {
            ...
            if (configuration.prefab == null)
                throw new System.ArgumentNullException("[PoolContainer] Prefab não pode ser nulo");
        ...
        {
            if (parent == null)
                throw new System.ArgumentNullException("[PoolContainer] Transform pai não pode ser nulo");
        }
        #endregion
        ...
        /// <summary>
        /// Cria um novo objeto para o pool com configuração completa
        /// </summary>
        ...
        /// <summary>
        /// Ativa o objeto e define seu estado
        /// </summary>
        ...
        bool IsValidAvailableObject(int index)
        {
            return index >= 0 && 
                   ...
                   allObjects[index].gameObject != null &&
        ...
        {
            if (poolableObject == null || poolableObject.IsInPool || isDisposed)
                return false;
        ...
        List<PoolableObject> CollectActiveObjects()
        {
            ...
            for (int i = 0; i < allObjects.Count; i++)
            {
                ...
                if (poolableObject != null && poolableObject.IsActive)
        ...
        int ProcessValidationAndDestroy(NativeArray<bool> validationResults)
        {
            ...
            for (int i = 0; i < allObjects.Count; i++)
            {
                var poolableObject = allObjects[i];
                if (poolableObject != null && 
                    poolableObject.gameObject != null && 
                    validationResults[i])
                {
                    Object.Destroy(poolableObject.gameObject);
                    destroyedCount++;
        ...
        string GetNextObjectName()
        {
            if (sequentialQueue.Count > 0)
            {
                ...
                if (nextIndex < allObjects.Count && allObjects[nextIndex] != null)
                    return allObjects[nextIndex].name;
            }

            if (nextSequentialIndex < allObjects.Count && allObjects[nextSequentialIndex] != null)
